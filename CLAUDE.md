...
A/B-compares versions (you can *hear* what changed between saves). v1 is **local only, no cloud.**
...
- **No "learn X first" gates.** New bits (Electron IPC, etc.) are learned *while building*.
...
effort** — the entire point is *shipping it*, not perfecting it. His known failure modes are
