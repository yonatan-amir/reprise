...
export function createProject(db: DatabaseSync, input: ProjectInput): Project {
  ...
  db.prepare('INSERT INTO projects (id, name, watchedPath) VALUES (?, ?, ?)').run(
    project.id,
    project.name,
    project.watchedPath
  )
...
export function listProjects(db: DatabaseSync): Project[] {
  const rows = db.prepare('SELECT id, name, watchedPath FROM projects').all()
...
export function getProject(db: DatabaseSync, id: string): Project | null {
  const row = db.prepare('SELECT id, name, watchedPath FROM projects WHERE id = ?').get(id)
